...
void aes128_key_expand(aes128_ctx *ctx, const uint8_t key[AES128_KEY_SIZE]) {
    ...
    for (int i = 4; i < 4 * (AES128_ROUNDS + 1); ++i) {
        uint32_t t = rk[i - 1];
        if (i % 4 == 0) {
...
// (FIPS 197 section 5.1); the final round omits MixColumns.
void aes128_encrypt_block(const aes128_ctx *ctx, const uint8_t in[AES128_BLOCK_SIZE],
    ...
    rk += 4 * AES128_ROUNDS;
    t0 = (TE4[(s0 >> 24)] & 0xff000000u) ^ (TE4[(s1 >> 16) & 0xff] & 0x00ff0000u) ^
         (TE4[(s2 >> 8) & 0xff] & 0x0000ff00u) ^ (TE4[s3 & 0xff] & 0x000000ffu) ^ rk[0];
    t1 = (TE4[(s1 >> 24)] & 0xff000000u) ^ (TE4[(s2 >> 16) & 0xff] & 0x00ff0000u) ^
         (TE4[(s3 >> 8) & 0xff] & 0x0000ff00u) ^ (TE4[s0 & 0xff] & 0x000000ffu) ^ rk[1];
    t2 = (TE4[(s2 >> 24)] & 0xff000000u) ^ (TE4[(s3 >> 16) & 0xff] & 0x00ff0000u) ^
         (TE4[(s0 >> 8) & 0xff] & 0x0000ff00u) ^ (TE4[s1 & 0xff] & 0x000000ffu) ^ rk[2];
    t3 = (TE4[(s3 >> 24)] & 0xff000000u) ^ (TE4[(s0 >> 16) & 0xff] & 0x00ff0000u) ^
         (TE4[(s1 >> 8) & 0xff] & 0x0000ff00u) ^ (TE4[s2 & 0xff] & 0x000000ffu) ^ rk[3];
...
void aes128_encrypt_ecb(const aes128_ctx *ctx, const uint8_t *in, size_t len, uint8_t *out) {
    for (size_t off = 0; off + AES128_BLOCK_SIZE <= len; off += AES128_BLOCK_SIZE) {
        aes128_encrypt_block(ctx, in + off, out + off);
    }
}
