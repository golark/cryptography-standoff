//! Scalar AES-128 (T-table design), mirroring c/src/aes128.c.
...
// x^i in GF(2^8), FIPS 197 section 5.3
...
impl Aes128 {
    /// FIPS 197 section 5.2: RotWord(SubWord(w)) ^ Rcon
    pub fn new(key: &[u8; KEY_SIZE]) -> Aes128 {
        let mut rk = [0u32; 4 * (ROUNDS + 1)];
        rk[..4].copy_from_slice(&[
            get_u32(&key[0..4]),
            get_u32(&key[4..8]),
            get_u32(&key[8..12]),
            get_u32(&key[12..16]),
        ...
        for i in 4..4 * (ROUNDS + 1) {
            let mut t = rk[i - 1];
            if i % 4 == 0 {
                t = ((SBOX[((t >> 16) & 0xff) as usize] as u32) << 24)
                    | ((SBOX[((t >> 8) & 0xff) as usize] as u32) << 16)
                    | ((SBOX[(t & 0xff) as usize] as u32) << 8)
                    | (SBOX[(t >> 24) as usize] as u32);
    ...
    /// Rounds 1..9 fuse SubBytes/ShiftRows/MixColumns into table lookups
    /// (FIPS 197 section 5.1); the final round omits MixColumns. Results go
    /// through temporaries: writing back into the state words in place would
    /// feed already-updated values to later columns.
    pub fn encrypt_block(&self, input: &[u8; BLOCK_SIZE], out: &mut [u8; BLOCK_SIZE]) {
        let rk = &self.rk;
        let mut s0 = get_u32(&input[0..4]) ^ rk[0];
        let mut s1 = get_u32(&input[4..8]) ^ rk[1];
        let mut s2 = get_u32(&input[8..12]) ^ rk[2];
        let mut s3 = get_u32(&input[12..16]) ^ rk[3];
        ...
        let b = 4 * ROUNDS;
        let f = |w: [u32; 4], k: u32| -> u32 {
            (TE4[(w[0] >> 24) as usize] & 0xff000000)
                ^ (TE4[((w[1] >> 16) & 0xff) as usize] & 0x00ff0000)
                ^ (TE4[((w[2] >> 8) & 0xff) as usize] & 0x0000ff00)
                ^ (TE4[(w[3] & 0xff) as usize] & 0x000000ff)
                ^ k
        };
        let t0 = f([s0, s1, s2, s3], rk[b]);
        let t1 = f([s1, s2, s3, s0], rk[b + 1]);
        let t2 = f([s2, s3, s0, s1], rk[b + 2]);
        let t3 = f([s3, s0, s1, s2], rk[b + 3]);
    ...
    /// ECB mode; processes floor(len / 16) whole blocks.
    pub fn encrypt_ecb(&self, input: &[u8], out: &mut [u8]) {
...
mod tests {
    ...
    fn kat(key: &str, pt: &str, ct: &str) -> bool {
        let k: [u8; KEY_SIZE] = unhex(key).try_into().unwrap();
        let ctx = Aes128::new(&k);
        let p: [u8; BLOCK_SIZE] = unhex(pt).try_into().unwrap();
        let mut out = [0u8; BLOCK_SIZE];
        ctx.encrypt_block(&p, &mut out);
        out.to_vec() == unhex(ct)
    }
    ...
    #[test]
    fn kat_sp80038a_ecb_x4() {
        let k: [u8; KEY_SIZE] = unhex("2b7e151628aed2a6abf7158809cf4f3c")
            .try_into()
            .unwrap();
        let ctx = Aes128::new(&k);
        let pt = unhex(
